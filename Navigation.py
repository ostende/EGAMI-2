...
from os import path
from enigma import eServiceCentereServiceReferenceeTimerpNavigationgetBestPlayableServiceReferenceiPlayableServiceeStreamServer
...
from time import timectime
import RecordTimer
import PowerTimer
import Screens.Standby
import NavigationInstance
import ServiceReference
from Screens.InfoBar import InfoBarMoviePlayer
...
class Navigation:
    ...
    def __init__(self, wakeupData=None):
        ...
        try:
            ...
            self.wakeuptime, self.timertime, self.wakeuptyp, self.getstandby, self.recordtime, self.forcerecord = (-1,
                                                                                                                   -1,
                                                                                                                   0,
                                                                                                                   0,
                                                                                                                   -1,
        ...
        if self.wakeuptime > 0:
            ...
            print '-' * 100
        thisBox = getBoxType()
        if not config.workaround.deeprecord.value and (wasTimerWakeup_failure or thisBox in ('ixussone',
                                                                                             'uniboxhd1',
                                                                                             'uniboxhd2',
                                                                                             'uniboxhd3',
                                                                                             'sezam5000hd',
    ...
    def playService(self, ref, checkParentalControl=True, forceRestart=False, adjust=True):
        ...
        if ref and oldref and ref == oldref and not forceRestart:
            ...
            if path.exists('/proc/stb/lcd/symbol_signal') and config.lcd.mode.value == '1':
                ...
                if path.exists('/proc/stb/lcd/symbol_signal') and config.lcd.mode.value == '0':
                    f = open('/proc/stb/lcd/symbol_signal', 'w')
                    f.write('0')
                    f.close()
                if ref is None:

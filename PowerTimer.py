...
from time import ctimetimestrftimelocaltimemktime
from bisect import insort
from enigma import eActionMapquitMainloop
...
class PowerTimerEntry(timer.TimerEntry, object):
        ...
        if (next_state == self.StateRunning or next_state == self.StateEnded) and NavigationInstance.instance.PowerTimer is None:
            ...
            if next_state == self.StateRunning:
                ...
                if self.timerType == TIMERTYPE.STANDBY:
                    ...
                    prioPT = [
                     TIMERTYPE.WAKEUP, TIMERTYPE.RESTART, TIMERTYPE.REBOOT, TIMERTYPE.DEEPSTANDBY]
                ...
                if self.timerType == TIMERTYPE.AUTOSTANDBY:
                    ...
                    if self.timerType == TIMERTYPE.RESTART:
                        ...
                        prioPT = [
                         TIMERTYPE.RESTART, TIMERTYPE.REBOOT, TIMERTYPE.DEEPSTANDBY]
                    ...
                    if self.timerType == TIMERTYPE.REBOOT:
                        ...
                        prioPT = [
                         TIMERTYPE.REBOOT, TIMERTYPE.DEEPSTANDBY]
                        prioPTae = [AFTEREVENT.DEEPSTANDBY]
                        shiftPT, breakPT = self.getPriorityCheck(prioPT, prioPTae)
                    ...
                    if self.timerType == TIMERTYPE.DEEPSTANDBY:
                        ...
                        prioPT = [
                         TIMERTYPE.WAKEUP, TIMERTYPE.WAKEUPTOSTANDBY, TIMERTYPE.DEEPSTANDBY]
    ...
    def getNextActivation(self):
        if self.state == self.StateEnded or self.state == self.StateFailed:
            return self.end
        next_state = self.state + 1
        return {self.StatePrepared: self.start_prepare,self.StateRunning: self.begin,
    ...
    def getNextWakeup(self, getNextStbPowerOn=False):
        ...
        if getNextStbPowerOn:
            if next_state == 3 and (self.timerType == TIMERTYPE.WAKEUP or self.timerType == TIMERTYPE.WAKEUPTOSTANDBY or self.afterEvent == AFTEREVENT.WAKEUP or self.afterEvent == AFTEREVENT.WAKEUPTOSTANDBY):
                ...
                if next_state == 2 and (self.timerType == TIMERTYPE.WAKEUP or self.timerType == TIMERTYPE.WAKEUPTOSTANDBY):
                    return self.begin
                if next_state == 1 and (self.timerType == TIMERTYPE.WAKEUP or self.timerType == TIMERTYPE.WAKEUPTOSTANDBY):
                    return self.start_prepare
                if next_state < 3 and (self.afterEvent == AFTEREVENT.WAKEUP or self.afterEvent == AFTEREVENT.WAKEUPTOSTANDBY):
                    return self.end
                return -1
    ...
    def saveTimer(self):
        ...
        for timer in self.timer_list + self.processed_timers:
            if timer.dontSave:
                continue
            list.append('<timer')
            list.append(' timertype="' + str(stringToXML({TIMERTYPE.NONE: 'nothing',TIMERTYPE.WAKEUP: 'wakeup',TIMERTYPE.WAKEUPTOSTANDBY: 'wakeuptostandby',TIMERTYPE.AUTOSTANDBY: 'autostandby',TIMERTYPE.AUTODEEPSTANDBY: 'autodeepstandby',TIMERTYPE.STANDBY: 'standby',TIMERTYPE.DEEPSTANDBY: 'deepstandby',TIMERTYPE.REBOOT: 'reboot',TIMERTYPE.RESTART: 'restart'}[timer.timerType])) + '"')
            list.append(' begin="' + str(int(timer.begin)) + '"')
            list.append(' end="' + str(int(timer.end)) + '"')
            list.append(' repeated="' + str(int(timer.repeated)) + '"')
            list.append(' afterevent="' + str(stringToXML({AFTEREVENT.NONE: 'nothing',AFTEREVENT.WAKEUP: 'wakeup',AFTEREVENT.WAKEUPTOSTANDBY: 'wakeuptostandby',AFTEREVENT.STANDBY: 'standby',AFTEREVENT.DEEPSTANDBY: 'deepstandby'}[timer.afterEvent])) + '"')
        ...
        for timer in self.timer_list:
            if timer.timerType != TIMERTYPE.AUTOSTANDBY and timer.timerType != TIMERTYPE.AUTODEEPSTANDBY:
                ...
                if nextPTlist[0][0] == -1:
                    ...
                    nextPTlist = [
                     (
                      next_act, next_timertype, next_afterevent, timer.state)]
                else:
                    if abs(next_act - timer.begin) <= 30:
                        next_timertype = timer.timerType
                    elif abs(next_act - timer.end) <= 30:
    ...
    def getNextPowerManagerTime(self, getNextStbPowerOn=False, getNextTimerTyp=False):
        ...
        if getNextStbPowerOn:
            if config.timeshift.isRecording.value:
                ...
                return (
                 nextrectime[0][0], int(nextrectime[0][1] == 2 or nextrectime[0][2] == 2))

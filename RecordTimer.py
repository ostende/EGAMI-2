...
from time import localtimestrftimectimetime
from bisect import insort
from sys import maxint
import os
from enigma import eEPGCachegetBestPlayableServiceReferenceeStreamServereServiceReferenceiRecordableServicequitMainloopeActionMapsetPreferredTunereServiceCenter
from Components.config import config
from Components import Harddisk
from Components.UsageConfig import defaultMoviePathcalcFrontendPriorityIntval
...
from Tools import DirectoriesNotificationsASCIItranslitTrashcan
from Tools.XMLTools import stringToXML
import timer
import NavigationInstance
from ServiceReference import ServiceReference
from enigma import pNavigationeDVBFrontend
...
class RecordTimerEntry(timer.TimerEntry, object):
        ...
        if next_state == self.StatePrepared:
            ...
            if self.first_try_prepare == 0:
                ...
                if InfoBarInstance and InfoBarPiP.pipShown(InfoBarInstance) == True:
                    ...
                    elif config.recording.ask_to_abort_pip.value in ('abort_no_msg',
                                                                     'abort_msg'):
                ...
                if len(NavigationInstance.instance.getRecordings(False, pNavigation.isPseudoRecording)) > 0:
                    if config.recording.ask_to_abort_pseudo_rec.value == 'ask':
                        ...
                        if InfoBar and InfoBar.instance:
                            ...
                            Notifications.AddNotificationWithCallback(callback, MessageBox, message, messageboxtyp, timeout=timeout, default=default)
                    elif config.recording.ask_to_abort_pseudo_rec.value in ('abort_no_msg',
                                                                            'abort_msg'):
                ...
                if len(NavigationInstance.instance.getRecordings(False, pNavigation.isStreaming)) > 0:
                    if config.recording.ask_to_abort_streaming.value == 'ask':
                        ...
                        if InfoBar and InfoBar.instance:
                            ...
                            Notifications.AddNotificationWithCallback(callback, MessageBox, message, messageboxtyp, timeout=timeout, default=default)
                    elif config.recording.ask_to_abort_streaming.value in ('abort_no_msg',
                                                                           'abort_msg'):
    ...
    def getNextActivation(self, getNextStbPowerOn=False):
        ...
        if next_state == self.StateEnded or next_state == self.StateFailed:
            if self.end > time():
                self.isStillRecording = True
        return {self.StatePrepared: self.start_prepare,self.StateRunning: self.begin,self.StateEnded: self.end
           }[next_state]
...
class RecordTimer(timer.Timer):
    ...
    def saveTimer(self):
        ...
        for timer in self.timer_list + self.processed_timers:
            ...
            list.append(' afterevent="' + str(stringToXML({AFTEREVENT.NONE: 'nothing',AFTEREVENT.STANDBY: 'standby',AFTEREVENT.DEEPSTANDBY: 'deepstandby',AFTEREVENT.AUTO: 'auto'}[timer.afterEvent])) + '"')
    ...
    def getNextRecordingTimeOld(self, getNextStbPowerOn=False):
        ...
        if getNextStbPowerOn:
            ...
            for timer in self.timer_list:
                ...
                if save_act[0] == -1:
                    save_act = (
                     next_act, int(not timer.always_zap))
    ...
    def isNextRecordAfterEventActionAuto--- This code section failed: ---

1297       0  SETUP_LOOP           76  'to 79'
           3  LOAD_FAST             0  'self'
           6  LOAD_ATTR             0  'timer_list'
           9  GET_ITER         
          10  FOR_ITER             65  'to 78'
          13  STORE_FAST            1  'timer'

1299      16  LOAD_GLOBAL           1  'True'
          19  RETURN_VALUE     

1300      20  LOAD_FAST             1  'timer'
          23  LOAD_ATTR             2  'justplay'
          26  POP_JUMP_IF_FALSE    35  'to 35'

1301      29  CONTINUE             10  'to 10'
          32  JUMP_FORWARD          0  'to 35'
        35_0  COME_FROM                '32'

1302      35  LOAD_FAST             1  'timer'
          38  LOAD_ATTR             3  'afterEvent'
          41  LOAD_GLOBAL           4  'AFTEREVENT'
          44  LOAD_ATTR             5  'AUTO'
          47  COMPARE_OP            2  '=='
          50  POP_JUMP_IF_TRUE     71  'to 71'
          53  LOAD_FAST             1  'timer'
          56  LOAD_ATTR             3  'afterEvent'
          59  LOAD_GLOBAL           4  'AFTEREVENT'
          62  LOAD_ATTR             6  'DEEPSTANDBY'
          65  COMPARE_OP            2  '=='
        68_0  COME_FROM                '50'
          68  POP_JUMP_IF_FALSE    10  'to 10'

1303      71  LOAD_GLOBAL           1  'True'
          74  RETURN_END_IF    
        75_0  COME_FROM                '68'
          75  JUMP_BACK            10  'to 10'
          78  POP_BLOCK        
        79_0  COME_FROM                '0'

1304      79  LOAD_GLOBAL           7  'False'
          82  RETURN_VALUE     
          -1  RETURN_LAST      

Parse error at or near `LOAD_FAST' instruction at offset 35
        ...
        for x in self.timer_list:
            ...
            if check:
                ...
                if time_match:
                    if getTimer:
                        returnValue = (
                         time_match, type, isAutoTimer, x)
                    else:
                        returnValue = (
                         time_match, type, isAutoTimer)
                    if type in (2, 7, 12):
                        break

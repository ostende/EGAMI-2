...
from enigma import eSizeePointeRectgFonteWindoweLabelePixmapeCursoreFrameeBareWindowStyleManageraddFontgRGBeWindowStyleSkinnedgetDesktop
from Components.config import ConfigSubsectionConfigTextConfigSliderconfigConfigSelectionConfigNothing
from Components.Sources.Source import ObsoleteSource
from Components.SystemInfo import SystemInfo
from Tools.Directories import resolveFilenameSCOPE_SKINSCOPE_SKIN_IMAGESCOPE_FONTSSCOPE_ACTIVE_SKINSCOPE_ACTIVE_LCDSKINSCOPE_CURRENT_SKINSCOPE_CONFIGfileExists
...
def parseValuePair(s, scale, object=None, desktop=None, size=None):
    ...
    return (
     xval * scale[0][0] / scale[0][1], yval * scale[1][0] / scale[1][1])
...
class AttributeParser():
    ...
    def __init__(self, guiObject, desktop, scale=(
 (1, 1), (1, 1))):
    ...
    def Animation(self, value):
        self.guiObject.setAnimationMode({'disable': 0,'off': 0,
           'offshow': 16,
           'offhide': 1,
           'onshow': 1,
           'onhide': 16,
    ...
    def alphatest(self, value):
        self.guiObject.setAlphatest({'on': 1,'off': 0,
           'blend': 2
           }[value])
    ...
    def orientation(self, value):
        try:
            self.guiObject.setOrientation(*{'orVertical': (self.guiObject.orVertical, False),'orTopToBottom': (
                               self.guiObject.orVertical, False),
               'orBottomToTop': (
                               self.guiObject.orVertical, True),
               'orHorizontal': (
                              self.guiObject.orHorizontal, False),
               'orLeftToRight': (
                               self.guiObject.orHorizontal, False),
               'orRightToLeft': (
                               self.guiObject.orHorizontal, True)
               }[value])
        except KeyError:
    ...
    def valign(self, value):
        try:
            self.guiObject.setVAlign({'top': self.guiObject.alignTop,'center': self.guiObject.alignCenter,
    ...
    def halign(self, value):
        try:
            self.guiObject.setHAlign({'left': self.guiObject.alignLeft,'center': self.guiObject.alignCenter,
...
def applySingleAttribute(guiObject, desktop, attrib, value, scale=(
 (
  1, 1), (1, 1))):
...
def loadSingleSkinData(desktop, skin, path_prefix):
    ...
    for c in skin.findall('cascadingstylesheets'):
        ...
        for pixmap in c.findall('css'):
            ...
            if name in ('pixmap', 'pointer', 'seek_pointer', 'backgroundPixmap', 'selectionPixmap',
                        'sstartPixmap', 'sbodyPixmap', 'sendPixmap', 'sballPixmap',
                        'sbackgroundPixmap', 'sliderBackground', 'sliderForeground',
                        'sliderPointer', 'tipTop', 'tipBottom', 'tipLeft', 'tipReight'):
    ...
    def process_widget(widget, context):
        ...
        if wname is None and wsource is None:
            ...
            if wname and wsource is None:
                ...
                collectAttributes(attributes, widget, context, skin_path_prefix, ignore=('render',
                                                                                         'source',
                                                                                         'name'))

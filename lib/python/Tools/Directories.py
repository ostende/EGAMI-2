...
PATH_DONTCREATE = 1
PATH_FALLBACK = 2
defaultPaths = {SCOPE_TRANSPONDERDATA: (
                         eEnv.resolve('${sysconfdir}/'), PATH_DONTCREATE),
   SCOPE_SYSETC: (
                eEnv.resolve('${sysconfdir}/'), PATH_DONTCREATE),
   SCOPE_FONTS: (
               eEnv.resolve('${datadir}/fonts/'), PATH_DONTCREATE),
   SCOPE_CONFIG: (
                eEnv.resolve('${sysconfdir}/enigma2/'), PATH_CREATE),
   SCOPE_PLUGINS: (
                 eEnv.resolve('${libdir}/enigma2/python/Plugins/'), PATH_CREATE),
   SCOPE_LANGUAGE: (
                  eEnv.resolve('${datadir}/enigma2/po/'), PATH_DONTCREATE),
   SCOPE_SKIN: (
              eEnv.resolve('${datadir}/enigma2/'), PATH_DONTCREATE),
   SCOPE_LCDSKIN: (
                 eEnv.resolve('${datadir}/enigma2/display/'), PATH_DONTCREATE),
   SCOPE_SKIN_IMAGE: (
                    eEnv.resolve('${datadir}/enigma2/'), PATH_DONTCREATE),
   SCOPE_HDD: (
             '/media/hdd/movie/', PATH_DONTCREATE),
   SCOPE_TIMESHIFT: (
                   '/media/hdd/timeshift/', PATH_DONTCREATE),
   SCOPE_VOD: (
             '/media/hdd/vod/', PATH_DONTCREATE),
   SCOPE_AUTORECORD: (
                    '/media/hdd/movie/', PATH_DONTCREATE),
   SCOPE_MEDIA: (
               '/media/', PATH_DONTCREATE),
   SCOPE_PLAYLIST: (
                  eEnv.resolve('${sysconfdir}/enigma2/playlist/'), PATH_CREATE),
   SCOPE_USERETC: (
                 '', PATH_DONTCREATE),
   SCOPE_DEFAULTDIR: (
                    eEnv.resolve('${datadir}/enigma2/defaults/'), PATH_CREATE),
   SCOPE_DEFAULTPARTITION: (
                          '/dev/mtdblock6', PATH_DONTCREATE),
   SCOPE_DEFAULTPARTITIONMOUNTDIR: (
                                  eEnv.resolve('${datadir}/enigma2/dealer'), PATH_CREATE),
   SCOPE_METADIR: (
                 eEnv.resolve('${datadir}/meta'), PATH_CREATE)
   }
FILE_COPY = 0
FILE_MOVE = 1
PATH_COPY = 2
PATH_MOVE = 3
fallbackPaths = {SCOPE_CONFIG: [
                (
                 '/home/root/', FILE_MOVE),
                (
   ...
   SCOPE_HDD: [
             (
              '/media/hdd/movie', PATH_MOVE)],
   SCOPE_TIMESHIFT: [
                   (
                    '/media/hdd/timeshift', PATH_MOVE)],
   SCOPE_VOD: [
             (
              '/media/hdd/vod', PATH_MOVE)],
   SCOPE_AUTORECORD: [
                    (
                     '/media/hdd/movie', PATH_MOVE)]
   }

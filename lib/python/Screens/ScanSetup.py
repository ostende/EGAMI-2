...
from Components.config import configConfigSubsectionConfigSelectionConfigYesNoConfigIntegergetConfigListEntryConfigSliderConfigEnableDisableinteger_limits
from Components.ActionMap import NumberActionMapActionMap
from Components.ConfigList import ConfigListScreen
from Components.NimManager import nimmanagergetConfigSatlist
from Components.Label import Label
from Components.Sources.StaticText import StaticText
from Tools.HardwareInfo import HardwareInfo
from Screens.InfoBar import InfoBar
from Screens.MessageBox import MessageBox
from enigma import eTimereDVBFrontendParametersSatelliteeComponentScaneDVBFrontendParametersTerrestrialeDVBFrontendParametersCableeDVBFrontendParametersATSCeConsoleAppContainereDVBResourceManageriDVBFrontend
...
cable_bands = {'DVBC_BAND_EU_VHF_I': 1 << 0,'DVBC_BAND_EU_MID': 1 << 1,
        ...
        for line in lines:
            ...
            if len(data):
                if data[0] == 'OK':
                    print str
                    parm = eDVBFrontendParametersCable()
                    qam = {'QAM16': parm.Modulation_QAM16,'QAM32': parm.Modulation_QAM32,
                       'QAM64': parm.Modulation_QAM64,
                       'QAM128': parm.Modulation_QAM128,
                       'QAM256': parm.Modulation_QAM256,
                       'QAM_AUTO': parm.Modulation_Auto
                       }
                    inv = {'INVERSION_OFF': parm.Inversion_Off,'INVERSION_ON': parm.Inversion_On,
                       'INVERSION_AUTO': parm.Inversion_Unknown
                       }
                    fec = {'FEC_AUTO': parm.FEC_Auto,'FEC_1_2': parm.FEC_1_2,
...
class TerrestrialTransponderSearchSupport():
    ...
    def terrestrialTransponderConvBandwidth_I(self, _bandWidth):
        bandWidth = {eDVBFrontendParametersTerrestrial.Bandwidth_8MHz: 8000000,eDVBFrontendParametersTerrestrial.Bandwidth_7MHz: 7000000,
...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport, TerrestrialTransponderSearchSupport):
    ...
    def createConfig(self):
        ...
        for n in nimmanager.nim_slots:
            ...
            if dvbc or dvbt or atsc:
                ...
                if n.canBeCompatible('DVB-S') and len(nimmanager.getSatListForNim(n.slot)) < 1:
                    continue
                if n.canBeCompatible('DVB-S') and n.config.dvbs.configMode.value in ('loopthrough',
                                                                                     'satposdepends'):
        ...
        self.scan_sat.frequency = ConfigInteger(default=defaultSat['frequency'], limits=(1,
                                                                                         99999))
        ...
        self.scan_sat.symbolrate = ConfigInteger(default=defaultSat['symbolrate'], limits=(1,
                                                                                           99999))
        ...
        self.scan_sat.is_id = ConfigInteger(default=defaultSat.get('is_id', 0), limits=(0,
                                                                                        255))
        ...
        self.scan_sat.pls_code = ConfigInteger(default=defaultSat.get('pls_code', 1), limits=(0,
                                                                                              262142))
        self.scan_cab.frequency = ConfigInteger(default=defaultCab['frequency'], limits=(50,
                                                                                         999))
        ...
        self.scan_cab.symbolrate = ConfigInteger(default=defaultCab['symbolrate'], limits=(1,
                                                                                           99999))
        ...
        self.scan_ter.frequency = ConfigFrequency(default=defaultTer['frequency'], limits=(50000,
                                                                                           999000))
        ...
        self.scan_ter.plp_id = ConfigInteger(default=defaultTer['plp_id'], limits=(0,
                                                                                   255))
        self.scan_ats.frequency = ConfigInteger(default=defaultATSC['frequency'], limits=(50000,
                                                                                          999000))
    ...
    def predefinedTranspondersList(self, orbpos):
        ...
        if orbpos is not None:
            ...
            if self.scan_sat.system.value == eDVBFrontendParametersSatellite.System_DVB_S2:
                ...
                fec = self.scan_sat.fec.value
            compare = [
             0, self.scan_sat.frequency.value * 1000, self.scan_sat.symbolrate.value * 1000, self.scan_sat.polarization.value, fec]
            i = 0
            tps = nimmanager.getTransponders(orbpos)
            for tp in tps:
                if tp[0] == 0:
    ...
    def predefinedCabTranspondersList(self):
        ...
        list = []
        compare = [
         1, self.scan_cab.frequency.value * 1000, self.scan_cab.symbolrate.value * 1000, self.scan_cab.modulation.value, self.scan_cab.fec.value, self.scan_cab.inversion.value, self.scan_cab.system.value]
        i = 0
        index_to_scan = int(self.scan_nims.value)
        tps = nimmanager.getTranspondersCable(index_to_scan)
        for tp in tps:
    ...
    def humanReadableCabTransponder(self, tp):
        if tp[3] in range(7) and (tp[4] in range(10) or tp[4] == 15):
            mod_list = [
             'Auto', '16-QAM', '32-QAM', '64-QAM', '128-QAM', '256-QAM', 'Auto']
...
class ScanSimple(ConfigListScreen, Screen, CableTransponderSearchSupport, TerrestrialTransponderSearchSupport):
    ...
    def __init__(self, session):
        ...
        for nim in nimmanager.nim_slots:
            ...
            if nim.canBeCompatible('DVB-T') and nim.config.dvbt.configMode.value != 'nothing':
                new_network = [
                 nimmanager.getTerrestrialDescription(nim.slot)]
            ...
            if nim.canBeCompatible('DVB-C') and nim.config.dvbc.configMode.value != 'nothing':
                new_network = [
                 nimmanager.getCableDescription(nim.slot)]
            ...
            if nim.canBeCompatible('ATSC') and nim.config.atsc.configMode.value != 'nothing':
                new_network = [
                 nimmanager.getATSCDescription(nim.slot)]

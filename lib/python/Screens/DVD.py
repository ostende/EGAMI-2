...
import os
from enigma import eTimeriPlayableServiceiServiceInformationeServiceReferenceiServiceKeysgetDesktop
from Screens.Screen import Screen
from Screens.MessageBox import MessageBox
from Screens.ChoiceBox import ChoiceBox
from Screens.HelpMenu import HelpableScreen
from Screens.InfoBarGenerics import InfoBarSeekInfoBarPVRStateInfoBarCueSheetSupportInfoBarShowHideInfoBarNotificationsInfoBarAudioSelectionInfoBarSubtitleSupport
...
from Tools.Directories import pathExistsfileExists
...
class DVDPlayer(Screen, InfoBarBase, InfoBarNotifications, InfoBarSeek, InfoBarPVRState, InfoBarShowHide, HelpableScreen, InfoBarCueSheetSupport, InfoBarAudioSelection, InfoBarSubtitleSupport):
    ...
    def __init__(self, session, dvd_device=None, dvd_filelist=[], args=None):
        ...
        self['DVDPlayerPlaybackActions'] = HelpableActionMap(self, 'DVDPlayerActions', {'dvdMenu': (
                     self.enterDVDMenu, _('show DVD main menu')),
           'toggleInfo': (
                        self.toggleInfo, _('toggle time, chapter, audio, subtitle info')),
           'nextChapter': (
           ...
           'tv': (
                self.askLeavePlayer, _('exit DVD player or return to file browser')),
           'dvdAudioMenu': (
           ...
           'nextSubtitleTrack': (
                               self.nextSubtitleTrack, _('switch to the next subtitle language')),
           'nextAngle': (
                       self.nextAngle, _('switch to the next angle')),
           'seekBeginning': self.seekBeginning
           }, -2)
        self['DVDPlayerColorActions'] = HelpableActionMap(self, 'ColorActions', {'blue': (
                  self.chapterZap, _('jump to chapter by number'))
           }, -2)
    ...
    def change_infobar_seek_config(self):
        config.seek.speeds_forward.value = [
         2, 4, 6, 8, 16, 32, 64]
    ...
    def askLeavePlayer(self):
        if self.autoplay:
            ...
            choices = [
             (
              _('Exit'), 'exit'), (_('Continue playing'), 'play')]
            if self.physicalDVD:
    ...
    def FileBrowserClosed(self, val):
        ...
        if val is None:
            ...
            if curref is None or curref != newref:
                ...
                files = [
                 ('/VIDEO_TS.IFO', 256), ('/VTS_01_0.IFO', 256), ('/VTS_01_0.IFO', 512)]
                for name in files:

...
from Components.config import configConfigSubsectionConfigText
from Components.Label import Label
from Components.ChoiceList import ChoiceEntryComponentChoiceList
...
class ChoiceBox(Screen):
    ...
    def __init__(self, session, title='', list=None, keys=None, selection=0, skin_name=None, text='', reorderConfig='', var='', windowTitle=None, allow_cancel=True, titlebartext=_('Choice Box')):
        ...
        self['text'] = Label()
        self.var = ''
        if skin_name and 'SoftwareUpdateChoices' in skin_name and var and var in ('unstable',
                                                                                  'updating',
                                                                                  'stable',
                                                                                  'unknown'):
            self.var = var
        ...
        if keys is None:
            self.__keys = [
             '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'red', 'green', 'yellow', 'blue', 'text'] + (len(list) - 10) * ['']
        else:
        ...
        self['actions'] = NumberActionMap(['WizardActions', 'InputActions', 'ColorActions', 'DirectionActions', 'MenuActions'], {'ok': self.go,
        ...
        self['cancelaction'] = NumberActionMap(['WizardActions', 'InputActions', 'ColorActions'], {'back': self.cancel
           }, -1)
        self.onShown.append(self.onshow)
        return

    def onshow(self):
        if self.skinName and 'SoftwareUpdateChoices' in self.skinName and self.var and self.var in ('unstable',
                                                                                                    'updating',
                                                                                                    'stable',
                                                                                                    'unknown'):
    ...
    def autoResize(self):
        ...
        if not self['text'].text:
            ...
            if textsize[0] < textsize[1]:
                textsize = (
                 textsize[1], textsize[0] + 10)
            if textsize[0] > 520:
                textsize = (
                 textsize[0], textsize[1] + itemheight)
            else:
                textsize = (
                 520, textsize[1] + itemheight)
            listsize = (
             textsize[0], itemheight * count)

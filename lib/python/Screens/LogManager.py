...
from os import pathremovewalkstatrmdir
from time import time
from datetime import datetime
from enigma import eTimereBackgroundFileErasereLabelgetDesktopgFontfontRenderClass
...
class LogManagerPoller():
    ...
    def JobTrash(self):
        ...
        f.close()
        if datetime.now().hour == 3 or time() - config.crash.lastfulljobtrashtime.value > 86400:
        ...
        if seconds_since_0330am > 43200:
            self.TrashTimer.startLongTimer(int(86400 - seconds_since_0330am))
        else:
...
class LogManagerViewLog(Screen):
    ...
    def layoutFinished(self):
        ...
        if path.exists(self.logfile):
            ...
            self.log = [
             _('file can not displayed - file not found')]
...
class LogManagerFb(Screen):
    ...
    def __init__(self, session, logpath=None):
        ...
        self['actions'] = ActionMap(['ChannelSelectBaseActions', 'WizardActions', 'DirectionActions', 'MenuActions', 'NumberActions', 'ColorActions'], {'ok': self.ok,
           'back': self.exit,
           'up': self.goUp,
           'down': self.goDown,
           'left': self.goLeft,

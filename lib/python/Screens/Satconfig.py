...
from enigma import eDVBDBeDVBResourceManagergetLinkedSlotIDisFBCLink
...
from Components.SelectionList import SelectionListSelectionEntryComponent
from Components.config import getConfigListEntryconfigconfigfileConfigNothingConfigSatlistConfigYesNoConfigSubsectionConfigSelection
...
from time import mktimelocaltime
...
class NimSetup(Screen, ConfigListScreen, ServiceStopScreen):
    ...
    def createSetup(self):
        ...
        if self.nim.isCompatible('DVB-S'):
            ...
            if nimConfig.configMode.value == 'simple':
                ...
                if nimConfig.diseqcMode.value in ('single', 'toneburst_a_b', 'diseqc_a_b',
                                                  'diseqc_a_b_c_d'):
        ...
        if self.nim.canBeCompatible('DVB-S'):
            if self.nimConfig.dvbs.configMode.value == 'simple':
                ...
                if self.nimConfig.dvbs.diseqcMode.value == 'single':
                    ...
                    if self.nimConfig.dvbs.diseqcMode.value == 'diseqc_a_b':
    ...
    def __init__(self, session, slotid):
        ...
        self['key_blue'] = Label(_('Client mode'))
        self['description'] = Label(' ')
    ...
    def keyLeft(self):
        ...
        if cur and isFBCLink(self.nim.slot):
            checkList = (
             self.advancedLof, self.advancedConnected)
            if cur in checkList:
                return
...
class NimSelection(Screen):
    ...
    def __init__(self, session):
        ...
        if config.usage.setup_level.index >= 2:
            self['key_blue'] = StaticText(_('Detail'))
        else:
            self['key_blue'] = StaticText()
        self['actions'] = ActionMap(['SetupActions', 'ColorActions', 'MenuActions', 'ChannelSelectEPGActions'], {'ok': self.okbuttonClick,
           'info': self.extraInfo,
           'epg': self.extraInfo,
           'cancel': self.close,
           'red': self.close,
    ...
    def extraInfo(self):
        if config.usage.setup_level.index >= 2:
            cmdlist = []
            for nim in self.list:
                name = nim and nim[1]
                nim = nim and nim[3]
                if nim is not None and name is not None:
                    text = _('Capabilities: ') + eDVBResourceManager.getInstance().getFrontendCapabilities(nim.slot)
                    cmdlist.append("echo '%s'" % name)
                    cmdlist.append("echo '%s'" % text)
                    cmdlist.append("echo ''")
                    cmdlist.append("echo '%s'" % ('=' * 100))
                    cmdlist.append("echo ''")

            if len(cmdlist) != 0:
                cmdlist.append("echo '%s'" % _('Press OK to close the window.'))
                self.session.open(SmartConsole, title=_('Detail info'), cmdlist=cmdlist, progressmode=False)
        return
            ...
            if self.showNim(x):
                if x.isMultiType():
                    ...
                    if nimConfig.configMode.value in ('loopthrough', 'equal', 'satposdepends'):
                        text = {'loopthrough': _('Loop through to'),'equal': _('Equal to'),'satposdepends': _('Second cable of motorized LNB')
                        ...
                        if nimConfig.diseqcMode.value in ('single', 'toneburst_a_b',
                                                          'diseqc_a_b', 'diseqc_a_b_c_d'):
                            ...
                            if nimConfig.diseqcMode.value in ('toneburst_a_b', 'diseqc_a_b',
                                                              'diseqc_a_b_c_d'):
...
class SelectSatsEntryScreen(Screen):
    ...
    def sortBy(self):
        ...
        if len(lst) > 1:
            menu = [
             (
              _('Reverse list'), '2'), (_('Standard list'), '1')]

...
from enigma import eDVBVolumecontroleTimereDVBLocalTimeHandlereServiceReference
from boxbranding import getMachineBrandgetMachineNamegetBoxTypegetBrandOEM
...
from enigma import quitMainloopiRecordableService
...
class QuitMainloopScreen(Screen):
    ...
    def __init__(self, session, retvalue=1):
        ...
        text = {1: _('Your %s %s is shutting down') % (getMachineBrand(), getMachineName()),2: _('Your %s %s is rebooting') % (getMachineBrand(), getMachineName()),
...
class TryQuitMainloop(MessageBox):
    ...
    def __init__(self, session, retvalue=1, timeout=-1, default_yes=True):
        ...
        if reason and inStandby:
            ...
            text = {1: _('Really shutdown now?'),2: _('Really reboot now?'),3: _('Really restart now?'),

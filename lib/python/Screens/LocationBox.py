...
from Tools.Directories import pathExistscreateDirremoveDir
from Components.config import config
import os
from Tools.NumericalTextInput import NumericalTextInput
from Components.ActionMap import NumberActionMapHelpableActionMap
...
class LocationBox(Screen, NumericalTextInput, HelpableScreen):
    ...
    def __init__(self, session, text='', filename='', currDir=None, bookmarks=None, userMode=False, windowTitle=_('Select location'), minFree=None, autoAdd=False, editDir=False, inhibitDirs=None, inhibitMounts=None):
        ...
        self['WizardActions'] = LocationBoxActionMap(self, 'WizardActions', {'ok': (
                self.ok, _('select')),
           'back': (
        ...
        self['EPGSelectActions'] = LocationBoxActionMap(self, 'EPGSelectActions', {'prevService': (
                         self.switchToBookList, _('switch to bookmarks')),
           'nextService': (
                         self.switchToFileList, _('switch to filelist'))
           }, -2)
        self['MenuActions'] = LocationBoxActionMap(self, 'MenuActions', {'menu': (
                  self.showMenu, _('menu'))
           }, -2)
        ...
        if ret:
            ...
            if self.realBookmarks:
                ...
                if self.filename and not os.path.exists(ret):
                    menu = [
                     (
                      _('Create new folder and exit'), 'folder'), (_('Save and exit'), 'exit')]
    ...
    def showMenu(self):
        if not self.userMode and self.realBookmarks:
            if self.currList == 'filelist':
                menu = [(_('switch to bookmarks'), self.switchToBookList),
                 (

...
from Components.ActionMap import ActionMapNumberActionMap
from Components.Sources.List import List
from Components.Sources.StaticText import StaticText
from Components.config import configconfigfilegetConfigListEntry
from Components.ConfigList import ConfigListConfigListScreen
from Components.MenuList import MenuList
from Tools.LoadPixmap import LoadPixmap
from Tools.Directories import SCOPE_ACTIVE_SKINresolveFilenamefileExists
from enigma import eTimerRT_HALIGN_LEFTeListboxPythonMultiContentgFontgetDesktopeSizeePoint
...
class OscamInfo():
    ...
    ECMTIME = 5
    IP_PORT = 6
    HEAD = {NAME: _('Label'),PROT: _('Protocol'),CAID_SRVID: _('CAID:SrvID'),
       SRVNAME: _('Serv.Name'),ECMTIME: _('ECM-Time'),
...
class OscamInfoMenu(Screen):
    ...
    def buildMenu(self, mlist):
        keys = [
         'red', 'green', 'yellow', 'blue', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '']
        menuentries = []
        k = 0
        for t in mlist:
            res = [
...
class oscInfo(Screen, OscamInfo):
    ...
    def buildListEntry(self, listentry, heading=False):
        ...
        if not HDSKIN:
            self.fieldsize = [
             100, 130, 100, 150, 80, 130]
            self.startPos = [10, 110, 240, 340, 490, 570]
            useFont = 3
        else:
            self.fieldsize = [
             150 * f, 150 * f, 150 * f, 300 * f, 150 * f, 200 * f]
    ...
    def showData(self):
        ...
        if not isinstance(data, str):
            if self.what != 'l':
                heading = (
                 self.HEAD[self.NAME], self.HEAD[self.PROT], self.HEAD[self.CAID_SRVID],

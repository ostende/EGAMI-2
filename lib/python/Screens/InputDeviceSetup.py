...
from Components.InputDevice import iInputDevicesiRcTypeControl
from Components.Sources.StaticText import StaticText
from Components.Sources.List import List
from Components.config import configConfigYesNogetConfigListEntryConfigSelection
...
from boxbranding import getBoxTypegetMachineBrandgetMachineNamegetBrandOEM
boxtype = getBoxType()
...
class InputDeviceSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                    self.close, _('Exit input device selection.')),
           'ok': (
                self.okbuttonClick, _('Select input device.'))
           }, -2)
...
class InputDeviceSetup(Screen, ConfigListScreen):
    ...
    def __init__(self, session, device):
        ...
        devicenames = [
         _('%s %s front panel') % (getMachineBrand(), getMachineName()), _('%s %s front panel') % (getMachineBrand(), getMachineName()), _('%s %s remote control (native)') % (getMachineBrand(), getMachineName()), _('%s %s advanced remote control (native)') % (getMachineBrand(), getMachineName()), _('%s %s ir keyboard') % (getMachineBrand(), getMachineName()), _('%s %s ir mouse') % (getMachineBrand(), getMachineName())]
        self.createSetup()
        self.onLayoutFinish.append(self.layoutFinished)
        self.onClose.append(self.cleanup)
        return
...
class RemoteControlType(Screen, ConfigListScreen):
    if getBrandOEM() in ('broadmedia', 'octagon', 'odin', 'protek', 'ultramini') or getBoxType() in ('et7x00',
                                                                                                     'et8500',
                                                                                                     'et1x000'):
        rcList = [('0', _('Default')),
         (

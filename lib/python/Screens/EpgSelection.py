...
from time import localtimetimestrftimemktime
from enigma import eServiceReferenceeTimereServiceCenterePoint
from Screen import Screen
from Screens.HelpMenu import HelpableScreen
from Components.About import about
from Components.ActionMap import HelpableActionMapHelpableNumberActionMap
from Components.Button import Button
from Components.config import configconfigfileConfigClock
from Components.EpgList import EPGListEPGBouquetListTimelineTextEPG_TYPE_SINGLEEPG_TYPE_SIMILAREPG_TYPE_MULTIEPG_TYPE_ENHANCEDEPG_TYPE_INFOBAREPG_TYPE_INFOBARGRAPHEPG_TYPE_GRAPHMAX_TIMELINES
...
class EPGSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session, service=None, zapFunc=None, eventid=None, bouquetChangeCB=None, serviceChangeCB=None, EPGtype=None, StartBouquet=None, StartRef=None, bouquets=None):
        ...
        self['dialogactions'] = HelpableActionMap(self, 'WizardActions', {'back': (
                  self.closeChoiceBoxDialog, _('Close dialog'))
           }, -1)
        ...
        self['okactions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
           ...
           'OKLong': (
                    self.OKLong, _('Zap to channel and close (setup in menu)'))
           }, -1)
        ...
        self['recordingactions'] = HelpableActionMap(self, 'InfobarInstantRecord', {'ShortRecord': (
           ...
           'LongRecord': (
                        self.recButtonPressedLong, _('Add a zap timer for current event'))
           }, -1)
        ...
        if self.type == EPG_TYPE_SIMILAR:
            ...
            self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (
               ...
               'down': (
                      self.moveDown, _('Goto next channel'))
               }, -1)
            ...
            self['bouquetokactions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
               ...
               'OK': (
                    self.BouquetOK, _('Change to bouquet'))
               }, -1)
            ...
            self['bouquetcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (
               ...
               'down': (
                      self.moveBouquetDown, _('Goto next channel'))
               }, -1)
            ...
            self['input_actions'] = HelpableNumberActionMap(self, 'NumberActions', {'1': (
               ...
               '0': (
                   self.keyNumberGlobal, _('Move to home of list'))
               }, -1)
            self['input_actions'].csel = self
    ...
    def openIMDb(self):
        try:
            from Plugins.Extensions.IMDb.plugin import IMDBIMDBEPGSelection
            try:
                cur = self['list'].getCurrent()
                event = cur[0]
    ...
    def RecordTimerQuestion(self, manual=False):
        ...
        for timer in self.session.nav.RecordTimer.timer_list:
            ...
            if foundtimer:
                ...
                menu = [
                 (
                  _('Add Timer'), 'CALLFUNC', self.ChoiceBoxCB, self.doRecordTimer), (_('Add AutoTimer'), 'CALLFUNC', self.ChoiceBoxCB, self.addAutoTimerSilent)]
            ...
            if self.session.nav.getCurrentlyPlayingServiceOrGroup() and self.StartRef and self.session.nav.getCurrentlyPlayingServiceOrGroup().toString() != self.StartRef.toString():
                if self.zapFunc and self.StartRef and self.StartBouquet:
                    if self.type == EPG_TYPE_GRAPH and config.epgselection.graph_preview_mode.value or self.type == EPG_TYPE_MULTI and config.epgselection.multi_preview_mode.value or self.type in (EPG_TYPE_INFOBAR, EPG_TYPE_INFOBARGRAPH) and config.epgselection.infobar_preview_mode.value in ('1',
                                                                                                                                                                                                                                                                                                     '2') or self.type == EPG_TYPE_ENHANCED and config.epgselection.enhanced_preview_mode.value:

...
from Components.ActionMap import HelpableActionMapActionMapNumberActionMap
from Components.ChoiceList import ChoiceListChoiceEntryComponent
from Components.Console import Console
from Components.MenuList import MenuList
from Components.MovieList import MovieListresetMoviePlayStateAUDIO_EXTENSIONSDVD_EXTENSIONSIMAGE_EXTENSIONSmoviePlayState
from Components.DiskInfo import DiskInfo
from Tools.Trashcan import TrashInfo
from Components.Pixmap import PixmapMultiPixmap
from Components.Label import Label
from Components.PluginComponent import plugins
from Components.config import configConfigSubsectionConfigTextConfigIntegerConfigLocationsConfigSetConfigYesNoConfigSelectiongetConfigListEntryConfigSelectionNumber
...
from Tools.Directories import resolveFilenameSCOPE_HDD
from Tools.BoundFunction import boundFunction
import Tools.CopyFiles
import Tools.Trashcan
import NavigationInstance
import RecordTimer
from enigma import eServiceReferenceeServiceCentereTimereSizeiPlayableServiceiServiceInformationgetPrevAsciiCodeeRCInputpNavigation
import os
import time
...
def buildMovieLocationList(bookmarks):
    ...
    for p in Components.Harddisk.harddiskmanager.getMountedPartitions():
        ...
        if d in inlist:
            try:
                bookmarks[bookmarks.index((d, d))] = (
                 p.tabbedDescription(), d)
            except:
                pass
...
class MovieContextMenu(Screen, ProtectedScreen):
    ...
    def __init__(self, session, csel, service):
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions', 'NumberActions', 'MenuActions'], {'red': self.cancelClick,
...
class MovieSelection(Screen, HelpableScreen, SelectionEventInfo, InfoBarBase, ProtectedScreen):
    ...
    def __init__(self, session, selectedmovie=None, timeshiftEnabled=False):
        ...
        if config.movielist.useslim.value:
            self.skinName = [
             'MovieSelectionSlim', 'MovieSelection']
        else:
        ...
        self['InfobarActions'] = HelpableActionMap(self, 'InfobarActions', {'showMovies': (
                        self.doPathSelect, _('Select the movie path')),
           'showRadio': (
                       self.btn_radio, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_radio')),
           'showTv': (
                    self.btn_tv, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_tv')),
           'showText': (
                      self.btn_text, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_text'))
           })
        ...
        self['playbackActions'] = HelpableActionMap(self, 'MoviePlayerActions', {'leavePlayer': (
                         self.playbackStop, _('Stop')),
           'moveNext': (
                      self.playNext, _('Play next')),
           'movePrev': (
                      self.playPrev, _('Play previous')),
           'channelUp': (
                       self.moveToFirstOrFirstFile, _('Go to first movie or top of list')),
           'channelDown': (
                         self.moveToLastOrFirstFile, _('Go to first movie or last item'))
           })
        self['MovieSelectionActions'] = HelpableActionMap(self, 'MovieSelectionActions', {'contextMenu': (
                         self.doContext, _('Menu')),
           'showEventInfo': (
                           self.showEventInformation, _('Show event details'))
           })
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (
                 self.btn_red, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_red')),
           'green': (
                   self.btn_green, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_green')),
           'yellow': (
                    self.btn_yellow, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_yellow')),
           'blue': (
                  self.btn_blue, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_blue')),
           'redlong': (
                     self.btn_redlong, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_redlong')),
           'greenlong': (
                       self.btn_greenlong, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_greenlong')),
           'yellowlong': (
                        self.btn_yellowlong, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_yellowlong')),
           'bluelong': (
                      self.btn_bluelong, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_bluelong'))
           })
        self['FunctionKeyActions'] = HelpableActionMap(self, 'FunctionKeyActions', {'f1': (
                self.btn_F1, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_F1')),
           'f2': (
                self.btn_F2, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_F2')),
           'f3': (
                self.btn_F3, boundFunction(self.getinitUserDefinedActionsDescription, 'btn_F3'))
           })
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                    self.abort, _('Exit movie list')),
           'ok': (
                self.itemSelected, _('Select movie'))
           })
        self['DirectionActions'] = HelpableActionMap(self, 'DirectionActions', {'up': (
                self.keyUp, _('Go up the list')),
           'down': (
                  self.keyDown, _('Go down the list'))
           }, prio=-2)
        tPreview = _('Preview')
        ...
        self['SeekActions'] = HelpableActionMap(self, 'MovielistSeekActions', {'playpauseService': (
    ...
    def initUserDefinedActions(self):
        ...
        if userDefinedButtons is None:
            userDefinedActions = {'delete': _('Delete'),'move': _('Move'),
               'copy': _('Copy'),
               'reset': _('Reset'),
               'tags': _('Tags'),

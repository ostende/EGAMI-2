...
from Components.config import configConfigSubsectionConfigSelectionConfigSubListgetConfigListEntryKEY_LEFTKEY_RIGHTKEY_0ConfigNothingConfigPINConfigTextConfigYesNoNoSave
from Components.ConfigList import ConfigList
from Components.SystemInfo import SystemInfo
from Tools.Directories import fileExists
from os import path as os_pathremoveunlinkrenamechmodaccessX_OK
from enigma import eTimereDVBCI_UIeDVBCIInterfaces
from Tools.BoundFunction import boundFunction
from boxbranding import getBrandOEMgetBoxType
import time
MAX_NUM_CI = 4
...
class CiSelection(Screen):
    ...
    def updateState(self, slot):
        ...
        if state == 0:
            self.list[slotidx] = (
             _('no module found'), ConfigNothing(), 2, slot)
        elif state == 1:
            self.list[slotidx] = (
             _('init module'), ConfigNothing(), 2, slot)
        elif state == 2:
            appname = eDVBCI_UI.getInstance().getAppName(slot)

...
from os import path
from enigma import iPlayableServiceiServiceInformationeTimereServiceCentereServiceReferenceeDVBDB
from Screens.Screen import Screen
from Screens.ChannelSelection import FLAG_IS_DEDICATED_3D
from Components.About import about
from Components.SystemInfo import SystemInfo
from Components.ConfigList import ConfigListScreen
from Components.config import configconfigfilegetConfigListEntryConfigNothing
from Components.Label import Label
from Components.Sources.StaticText import StaticText
from Components.Pixmap import Pixmap
from Components.Sources.Boolean import Boolean
from Components.ServiceEventTracker import ServiceEventTracker
from Tools.Directories import resolveFilenameSCOPE_PLUGINS
...
class VideoSetup(Screen, ConfigListScreen):
        ...
        if config.av.videoport.value in ('HDMI', 'YPbPr', 'Scart-YPbPr') and not getAutoresPlugin_enabled():
            ...
            if config.av.autores.value in ('all', 'hd'):
                ...
                if '1080i' in iAVSwitch.modes_available or '1080p' in iAVSwitch.modes_available:
                    ...
                    elif '1080p' not in iAVSwitch.modes_available and '1080p50' not in iAVSwitch.modes_available:
    ...
    def confirm(self, confirmed):
        if not confirmed:
            if self.reset_mode == 1:
                ...
                try:
                    if self.current_mode not in iAVSwitch.modes_available:
    ...
    def apply(self):
        ...
        if config.av.autores.value in ('all', 'hd') and ((port, mode, rate) != self.last_good or (autores_sd, smart1080p) != self.last_good_extra):
         ...
         port, mode, rate) != self.last_good or config.av.autores.value == 'disabled' and self.last_good_autores != 'disabled':
            self.reset_mode = 1
            self.hw.setMode(port, mode, rate)
        elif config.av.autores.value in ('native', 'simple') and ((port, mode_sd, rate_sd) != self.last_good_autores_sd or (port, mode_hd, rate_hd) != self.last_good_autores_hd or (port, mode_fhd, rate_fhd) != self.last_good_autores_fhd or (
         port, mode_uhd, rate_uhd) != self.last_good_autores_uhd or (autores_24p, autores_1080i) != self.last_good_autores_extra or self.last_good_autores != config.av.autores.value or self.reset_mode == 1 or self.last_good_autores_unknownres != config.av.autores_unknownres.value and config.av.autores.value == 'native'):
            self.reset_mode = 2
        ...
        if video_height and video_width and video_pol and video_rate:
            ...
            if video_height != -1:
                if video_height > 720 or video_width > 1280:
                    ...
                    new_res = '720'
                elif 480 < video_height <= 576 or video_width > 720 or video_rate in (25000,
                                                                                      23976,
                                                                                      24000):
                    new_res = '576'
                else:
            ...
            if config_mode in ('PAL', 'NTSC'):
                ...
                if new_mode + new_rate in iAVSwitch.modes_available:
                    ...
                    if config_rate != 'multi' and int(new_rate) > int(config_rate):
                        new_rate = config_rate
                ...
                if video_height <= int(min_res):
                    if new_pol == 'i' and min_pol == 'p':
                        new_pol = min_pol
                    if min_rate != 'multi' and new_rate < int(min_rate):
                        new_rate = min_rate
                    new_res = min_res
                if video_height >= int(config_res) or int(new_res) >= int(config_res):
                    ...
                    if video_pol == 'p' and config_pol == 'i':
                        new_pol = config_pol
                    if config_rate != 'multi' and int(config_rate) < new_rate:
                        new_rate = config_rate
                new_rate = str(new_rate)
                if new_pol == 'p':
                ...
                if path.exists('/proc/stb/video/videomode_%shz' % new_rate) and config_rate == 'multi':
                    ...
                    if multi_videomode and current_mode != multi_videomode:
                        ...
                        write_mode = config_mode + new_rate
            if config.av.smart1080p.value != 'false' and config.av.autores.value in ('all',
                                                                                     'hd'):

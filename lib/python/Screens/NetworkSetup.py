...
from boxbranding import getBoxTypegetMachineBrandgetMachineName
from os import path as os_pathremoveunlinkrenamechmodaccessX_OK
from shutil import move
import time
import string
import sys
from random import Random
from enigma import eTimereConsoleAppContainer
from Screens.Screen import Screen
from Screens.MessageBox import MessageBox
from Screens.Standby import TryQuitMainloop
from Screens.HelpMenu import HelpableScreen
from Components.OnlineUpdateCheck import feedsstatuscheck
from Components.About import aboutgetVersionString
...
from Components.Label import LabelMultiColorLabel
from Components.ScrollLabel import ScrollLabel
from Components.Pixmap import PixmapMultiPixmap
from Components.MenuList import MenuList
from Components.config import configConfigSubsectionConfigYesNoConfigIPConfigTextConfigPasswordConfigSelectiongetConfigListEntryConfigNumberConfigLocationsNoSaveConfigMacText
from Components.ConfigList import ConfigListScreenConfigList
from Components.PluginComponent import plugins
from Components.FileList import MultiFileSelectList
from Components.ActionMap import ActionMapNumberActionMapHelpableActionMap
from Tools.Directories import fileExistsresolveFilenameSCOPE_PLUGINSSCOPE_ACTIVE_SKIN
...
class NetworkAdapterSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                    self.close, _('Exit network interface list')),
           'ok': (
                self.okbuttonClick, _('Select interface'))
           })
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (
           ...
           'green': (
                   self.okbuttonClick, _('Select interface')),
           'blue': (
                  self.openNetworkWizard, _('Use the network wizard to configure selected network adapter'))
           })
        self['DefaultInterfaceAction'] = HelpableActionMap(self, 'ColorActions', {'yellow': (
                    self.setDefaultInterface, [_('Set interface as the default Interface'), _('* Only available if more than one interface is active.')])
           })
    ...
    def buildInterfaceList(self, iface, name, default, active):
        ...
        return (
         iface, name, description, interfacepng, defaultpng, activepng, divpng)
...
class NameserverSetup(Screen, ConfigListScreen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self.createConfig()
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                    self.cancel, _('Exit nameserver configuration')),
           'ok': (
                self.ok, _('Activate current configuration'))
           })
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (
                 self.cancel, _('Exit nameserver configuration')),
           'green': (
                   self.ok, _('Activate current configuration')),
           'yellow': (
                    self.add, _('Add a nameserver entry')),
           'blue': (
                  self.remove, _('Remove a nameserver entry'))
           })
...
class NetworkMacSetup(Screen, ConfigListScreen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (
                 self.cancel, _('Exit MAC address configuration')),
           'green': (
                   self.ok, _('Activate MAC address configuration'))
           })
        self['actions'] = NumberActionMap(['SetupActions'], {'ok': self.ok
           }, -2)
        self.list = []
...
class AdapterSetup(Screen, ConfigListScreen, HelpableScreen):
    ...
    def __init__(self, session, networkinfo, essid=None):
        ...
        self.createConfig()
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                    self.keyCancel, _('Exit network adapter configuration')),
           'ok': (
                self.keySave, _('Activate network adapter configuration'))
           })
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (
                 self.keyCancel, _('Exit network adapter configuration')),
           'green': (
                   self.keySave, _('Activate network adapter configuration')),
           'blue': (
                  self.KeyBlue, _('Open nameserver configuration'))
           })
        self['actions'] = NumberActionMap(['SetupActions'], {'ok': self.keySave
           }, -2)
        self.list = []
    ...
    def createSetup(self):
        ...
        if self.activateInterfaceEntry.value:
            ...
            havewol = False
            if SystemInfo['WakeOnLAN'] and getBoxType() not in ('et10000', 'gb800seplus',
                                                                'gb800ueplus', 'gbultrase',
                                                                'gbultraue', 'gbultraueh',
                                                                'gbipbox', 'gbquad',
                                                                'gbx1', 'gbx2', 'gbx3',
                                                                'gbx3h'):
                havewol = True
...
class AdapterSetupConfiguration(Screen, HelpableScreen):
    ...
    def __init__(self, session, iface):
        ...
        self['WizardActions'] = HelpableActionMap(self, 'WizardActions', {'up': (
                self.up, _('Move up to previous entry')),
           'down': (
                  self.down, _('Move down to next entry')),
           'left': (
                  self.left, _('Move up to first entry')),
           'right': (
                   self.right, _('Move down to last entry'))
           })
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                    self.close, _('Exit network adapter setup menu')),
           'ok': (
                self.ok, _('Select menu entry'))
           })
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (
                 self.close, _('Exit network adapter setup menu'))
           })
    ...
    def genMainMenu(self):
        menu = [
         (
          _('Adapter settings'), 'edit'), (_('Nameserver settings'), 'dns'), (_('Network test'), 'test'), (_('Restart network'), 'lanrestart')]
        self.extended = None
        ...
        for p in plugins.getPlugins(PluginDescriptor.WHERE_NETWORKSETUP):
            ...
            if callFnc is not None:
                ...
                if p.__call__.has_key('WlanPluginEntry'):
                    ...
                    self.extendedSetup = (
                     'extendedSetup', menuEntryDescription, self.extended)
...
class NetworkAdapterTest(Screen):
    ...
    def __init__(self, session, iface):
        ...
        self['shortcutsgreen'] = ActionMap(['ShortcutActions'], {'green': self.KeyGreen
           }, -2)
        self['shortcutsgreen_restart'] = ActionMap(['ShortcutActions'], {'green': self.KeyGreenRestart
           }, -2)
        self['shortcutsyellow'] = ActionMap(['ShortcutActions'], {'yellow': self.KeyYellow
           }, -2)
...
class NetworkMountsMenu(Screen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                    self.close, _('Exit mounts setup menu')),
           'ok': (
                self.ok, _('Select menu entry'))
           })
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (
                 self.close, _('Exit networkadapter setup menu'))
           })
        self['actions'] = NumberActionMap(['WizardActions', 'ShortcutActions'], {'ok': self.ok,
           'back': self.close,
           'up': self.up,
...
class NetworkOpenvpn(Screen):
    ...
    def doInstall(self, callback, pkgname):
        self.message = self.session.open(MessageBox, _('Please wait...'), MessageBox.TYPE_INFO, enable_input=False)
        self.message.setTitle(_('Installing service'))
        self.Console.ePopen('/usr/bin/opkg install openvpn-sample ' + pkgname, callback)
    ...
    def doRemove(self, callback, pkgname):
        self.message = self.session.open(MessageBox, _('Please wait...'), MessageBox.TYPE_INFO, enable_input=False)
        self.message.setTitle(_('Removing service'))
        self.Console.ePopen('/usr/bin/opkg remove openvpn-sample ' + pkgname + ' --force-remove --autoremove', callback)
...
class NetworkSamba(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'ok': self.close,'menu': self.setupsamba,'back': self.close,'red': self.UninstallCheck,'green': self.SambaStartStop,'yellow': self.activateSamba,'blue': self.Sambashowlog})
...
class NetworkInadynLog(Screen):
    ...
    def __init__(self, session):
        ...
        self['infotext'] = ScrollLabel('')
        self['actions'] = ActionMap(['WizardActions', 'DirectionActions', 'ColorActions'], {'ok': self.close,'back': self.close,

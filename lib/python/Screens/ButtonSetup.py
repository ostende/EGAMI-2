...
from Components.config import configConfigSubsectionConfigTextConfigYesNo
...
from enigma import eServiceReferenceeActionMap
from Components.Label import Label
import os
ButtonSetupKeys = [
 (
  _('Red'), 'red', 'Infobar/openSingleServiceEPG'),
 (
...
class ButtonSetupSelect(Screen):
    ...
    def __init__(self, session, key, args=None):
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions', 'DirectionActions', 'KeyboardInputActions'], {'ok': self.keyOk,
           'cancel': self.cancel,
           'red': self.cancel,
           'green': self.save,
           'up': self.keyUp,
...
class InfoBarButtonSetup:
    ...
    def getKeyFunctions(self, key):
        if key in ('play', 'playpause', 'Stop', 'stop', 'pause', 'rewind', 'next',
                   'previous', 'fastforward', 'skip_back', 'skip_forward') and (self.__class__.__name__ == 'MoviePlayer' or hasattr(self, 'timeshiftActivated') and self.timeshiftActivated()):
            return False
...
from Screens.Standby import quitMainloopTryQuitMainloop
from Components.config import ConfigSubsectionConfigIntegerConfigTextgetConfigListEntryConfigSelectionConfigIPConfigYesNoConfigSequenceConfigNumberNoSaveConfigEnableDisableconfigfile
from os import path

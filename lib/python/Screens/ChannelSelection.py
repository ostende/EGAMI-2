...
from Components.ServiceList import ServiceListrefreshServiceList
from Components.ActionMap import NumberActionMapActionMapHelpableActionMap
...
from enigma import eActionMapeServiceReferenceeEPGCacheeServiceCentereRCInputeTimerePointeDVBDBiPlayableServiceiServiceInformationgetPrevAsciiCodeeEnvloadPNGeDVBLocalTimeHandler
from Components.config import configconfigfileConfigSubsectionConfigTextConfigYesNo
...
from RecordTimer import RecordTimerEntryAFTEREVENT
from TimerEntry import TimerEntryInstantRecordTimerEntry
from Screens.InputBox import InputBoxPinInput
from Screens.VirtualKeyBoard import VirtualKeyBoard
from Screens.ChoiceBox import ChoiceBox
from Screens.MessageBox import MessageBox
from Screens.ServiceInfo import ServiceInfo
from Screens.ButtonSetup import InfoBarButtonSetupButtonSetupActionMapgetButtonSetupFunctions
    ...
    def removeEntry(self):
        if self.removeFunction and self.csel.servicelist.getCurrent() and self.csel.servicelist.getCurrent().valid():
            if self.csel.confirmRemove:
                list = [
                 (
                  _('yes'), True), (_('no'), False), (_('yes') + ' ' + _('and never ask again this session'), 'never')]
        ...
        if newservice and newservice.valid():
            ...
            if newservice and newservice.valid():
                if self.session.pip.playService(newservice):
                    ...
                    self.session.pip.servicePath = self.csel.getCurrentServicePath()
                    self.session.pip.servicePath[1] = currentBouquet
                    if SystemInfo['LCDMiniTV'] and int(config.lcd.minitvpipmode.value) >= 1:
                        print '[LCDMiniTV] enable PIP'
                        f = open('/proc/stb/lcd/mode', 'w')
                        f.write(config.lcd.minitvpipmode.value)
                        f.close()
...
class ChannelSelectionEPG(InfoBarButtonSetup):
    ...
    def __init__(self):
        ...
        self['recordingactions'] = HelpableActionMap(self, 'InfobarInstantRecord', {'ShortRecord': (
           ...
           'LongRecord': (
                        self.doZapTimer, _('Add a zap timer for next event'))
           }, -1)
        self['dialogactions'] = ActionMap(['OkCancelActions'], {'cancel': self.closeChoiceBoxDialog
           })
    ...
    def nextBouquet(self):
        if 'reverseB' in config.usage.servicelist_cursor_behavior.value:
            if config.usage.channelbutton_mode.value == '0':
                ...
                self.servicelist.moveDown()
        elif config.usage.channelbutton_mode.value == '0':
            self.changeBouquet(+1)
        else:
            self.servicelist.moveUp()
...
class ChannelSelection(ChannelSelectionBase, ChannelSelectionEdit, ChannelSelectionEPG, SelectionEventInfo):
    ...
    def animateMenuOnShow(self):
        if config.misc.enableAnimationChSelection.value:
    ...
    def __onCreate(self):
        if config.usage.e1like_radio_mode.value:
            ...
            self.setModeTv()
        lastservice = eServiceReference(self.lastservice.value)
        if not lastservice.valid() or lastservice.type != eServiceReference.idDVB and time() < eDVBLocalTimeHandler.timeOK:
    ...
    def findServiceBouquet(self, matchFunc):
        ...
        if config.usage.multibouquet.value:
            ...
            if bouquet.valid() and bouquet.flags & eServiceReference.isDirectory:
                foundService = matchService(bouquet, serviceHandler, matchFunc)
        return (
         foundService, bouquet, rootbouquet)
...
class ChannelSelectionRadio(ChannelSelectionBase, ChannelSelectionEdit, ChannelSelectionEPG, InfoBarBase, SelectionEventInfo):
    ...
    def __init__(self, session, infobar):
        ...
        self['RdsActions'] = HelpableActionMap(self, 'InfobarRdsActions', {'startRassInteractive': (
                                  self.startRassInteractive, _('View Rass interactive...'))
           }, -1)
    ...
    def __init__(self, session, items=None, sel_item=0, mark_item=0, invert_items=False, redirect_buttons=False, wrap_around=True):
        ...
        for x in items:
            ...
            if picon != '':
                png = loadPNG(picon)
            if self.invertItems:
                self.list.insert(0, (x[1], cnt == mark_item and '\xc2\xbb' or '', x[0], eventName, descriptionName, durationTime, png, orbpos))
            else:
                self.list.append((x[1], cnt == mark_item and '\xc2\xbb' or '', x[0], eventName, descriptionName, durationTime, png, orbpos))
            cnt += 1

...
from enigma import eDBoxLCDeTimeriPlayableServicepNavigation
...
def SymbolsCheck(session, **kwargs):
    ...
    global POLLTIME
    if getBoxType() in ('tmtwin4k', 'mbmicrov2', 'revo4k', 'force3uhd', 'wetekplay',
                        'wetekplay2', 'wetekhub', 'ixussone', 'ixusszero', 'mbmicro',
                        'e4hd', 'e4hdhybrid', 'dm7020hd', 'dm7020hdv2', '9910lx',
                        '9911lx') or getMachineBuild() in ('dags7362', 'dags73625',
                                                           'dags5'):
        POLLTIME = 1
...
class SymbolsCheckPoller:
    ...
    def Recording(self):
        if fileExists('/proc/stb/lcd/symbol_circle'):
        ...
        elif getBoxType() in ('alphatriple', 'mixosf5', 'mixoslumi', 'mixosf7', 'gi9196m',
                              'sf3038'):
            recordings = len(NavigationInstance.instance.getRecordings(False, Components.RecordingConfig.recType(config.recording.show_rec_symbol_for_rec_types.getValue())))
            if recordings > 0:
        ...
        elif getBoxType() in ('wetekplay', 'wetekplay2', 'wetekhub', 'ixussone', 'ixusszero',
                              '9910lx', '9911lx'):
            recordings = len(NavigationInstance.instance.getRecordings(False, Components.RecordingConfig.recType(config.recording.show_rec_symbol_for_rec_types.getValue())))
            ...
            if recordings > 0:
                ...
                open('/proc/stb/fp/led_set', 'w').write('0xffffffff')
        elif getMachineBuild() in ('dags7362', 'dags73625', 'dags5') or getBoxType() in ('tmtwin4k',
                                                                                         'revo4k',
                                                                                         'force3uhd'):
            recordings = len(NavigationInstance.instance.getRecordings(False, Components.RecordingConfig.recType(config.recording.show_rec_symbol_for_rec_types.getValue())))
            self.blink = not self.blink

...
from Tools.Directories import resolveFilenameSCOPE_CONFIGfileExists
...
class ConfigElement(object):
    ...
    def addNotifier(self, notifier, initial_call=True, immediate_feedback=True, call_on_save_or_cancel=False, extra_args=None):
        ...
        if immediate_feedback:
            self.__notifiers[str(notifier)] = (
             notifier, self.value, call_on_save_or_cancel)
        else:
            self.__notifiers_final[str(notifier)] = (
             notifier, self.value, call_on_save_or_cancel)
        if initial_call:
            if extra_args:
...
class ConfigBoolean(ConfigElement):
    ...
    def __init__(self, default=False, descriptions={False: _('false'),True: _('true')}, grafic=True):
        ConfigElement.__init__(self)
    ...
    def getMulti(self, selected):
        ...
        if not self.grafic or not config.usage.boolean_graphic.value:
            if descr:
...
class ConfigDateTime(ConfigElement):
    ...
    def getMulti(self, selected):
        return (
         'text', strftime(self.formatstring, localtime(self.value)))
...
class ConfigSlider(ConfigElement):
    ...
    def getMulti(self, selected):
        self.checkValues()
        SliderForeground = SliderBackground = SliderPointer = None
        SliderLength = None
        SliderLengthTip = None
        from config import config
        if config.usage.slider_graphic.value:
            from skin import cascadingStyleSheets
            if cascadingStyleSheets.has_key('slider_settings'):
                if cascadingStyleSheets['slider_settings'].has_key('sliderBackground'):
                    SliderBackground = LoadPixmap(cached=True, path=cascadingStyleSheets['slider_settings']['sliderBackground'])
                if cascadingStyleSheets['slider_settings'].has_key('sliderForeground'):
                    SliderForeground = LoadPixmap(cached=True, path=cascadingStyleSheets['slider_settings']['sliderForeground'])
                if cascadingStyleSheets['slider_settings'].has_key('sliderPointer'):
                    SliderPointer = LoadPixmap(cached=True, path=cascadingStyleSheets['slider_settings']['sliderPointer'])
                if cascadingStyleSheets['slider_settings'].has_key('sliderLength') and cascadingStyleSheets['slider_settings']['sliderLength']:
                    SliderLength = int(cascadingStyleSheets['slider_settings']['sliderLength'])
                if cascadingStyleSheets['slider_settings'].has_key('sliderLengthTip') and cascadingStyleSheets['slider_settings']['sliderLengthTip']:
                    SliderLengthTip = int(cascadingStyleSheets['slider_settings']['sliderLengthTip'])
            if SliderForeground is not None and SliderBackground is not None:
                if SliderPointer is not None:
                    return ('slider',
                     self.value,
                     self.max,
                     SliderLength,
...
class ConfigSet(ConfigElement):
    ...
    def getMulti(self, selected):
        if not selected or self.pos == -1:
            ...
            len_val1 = len(val1)
            return (
             'mtext', val1 + chstr + val2, range(len_val1, len_val1 + len(chstr)))
        ...
        for l in lines:
            ...
            if l.startswith('config.Nims.'):
                ...
                if tmp[3] == 'cable':
                    ...
                    tmp[3] = 'dvbt.' + tmp[3]
                elif tmp[3] not in ('dvbs', 'dvbc', 'dvbt', 'multiType'):
                    tmp[3] = 'dvbs.' + tmp[3]
                name = '.'.join(tmp)
            names = name.split('.')
            base = configbase

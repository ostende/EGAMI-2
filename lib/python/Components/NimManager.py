...
from time import localtimemktime
from datetime import datetime
import xml.etree.cElementTree
from os import pathaccessF_OK
from enigma import eDVBSatelliteEquipmentControl as secClasseDVBSatelliteLNBParameters as lnbParameDVBSatelliteDiseqcParameters as diseqcParameDVBSatelliteSwitchParameters as switchParameDVBSatelliteRotorParameters as rotorParameDVBResourceManagereDVBDBeEnviDVBFrontendpNavigation
from Tools.HardwareInfo import HardwareInfo
from Tools.BoundFunction import boundFunction
from Components.About import about
from config import configConfigSubsectionConfigSelectionConfigFloatConfigSatlistConfigYesNoConfigIntegerConfigSubListConfigNothingConfigSubDictConfigOnOffConfigDateTimeConfigText
...
class SecConfigure():
    ...
    def getRoot(self, slotid, connto):
        visited = []
        while self.NimManager.getNimConfig(connto).dvbs.configMode.value in ('satposdepends',
                                                                             'equal',
                                                                             'loopthrough'):
        ...
        for x in range(1, 71):
            if len(lnbSat[x]) > 0:
                ...
                if dm != 'none':
                    ...
                    c = {'none': diseqcParam.SENDNO,'AA': diseqcParam.AA,
                       'AB': diseqcParam.AB,
                       'BA': diseqcParam.BA,
                       'BB': diseqcParam.BB
                       }
...
class NIM(object):
    ...
    def __init__(self, slot, type, description, has_outputs=True, internally_connectable=None, multi_type=None, frontend_id=None, i2c=None, is_empty=False, input_name=None):
        if not multi_type:
            multi_type = {}
        self.slot = slot
        if type not in ('DVB-S', 'DVB-C', 'DVB-T', 'DVB-S2', 'DVB-T2', 'DVB-C2', 'ATSC',
                        None):
...
class NimManager():
            ...
            if db.readSatellites(self.satList, self.satellites, self.transponders):
                ...
                def emergencyAid():
                    if not path.exists('/etc/enigma2/lamedb'):
                        ...
                        t2_sv3 = ('frequency', 'symbol_rate', 'polarization', 'fec_inner',
                                  'position', 'inversion', 'system', 'modulation',
                                  'rolloff', 'pilot')
                        t2_sv4 = ('frequency', 'symbol_rate', 'polarization', 'fec_inner',
                                  'position', 'inversion', 'flags', 'system', 'modulation',
                                  'rolloff', 'pilot')
                        tplist = []
    ...
    def enumerateNIMs(self):
        ...
        if config.clientmode.enabled.value:
            ...
            for id, entry in entries.items():
                ...
                if entry.has_key('name') and 'SI4768' in entry['name'] and entry.has_key('type') and 'C2' in entry['type']:
                    entry['type'] = 'DVB-C'
    ...
    def canDependOn(self, slotid):
        ...
        for nim in nimList[:]:
            if self.nim_slots[nim].canBeCompatible('DVB-S'):
                mode = self.getNimConfig(nim).dvbs
                nimHaveRotor = mode.configMode.value == 'simple' and mode.diseqcMode.value in ('positioner',
                                                                                               'positioner_select')
...
jess_alias = (
 'JESS', 'UNICABLE2', 'SCD2', 'EN50607', 'EN 50607')
...
def InitNimManager(nimmgr, update_slots=[]):
    ...
    lnb_choices = {'universal_lnb': _('Universal LNB'),'unicable': _('Unicable / JESS'),
       'c_band': _('C-Band'),
    ...
    advanced_lnb_diction_user_choices = [
     ('EN50494', 'Unicable(EN50494)'), ('EN50607', 'JESS(EN50607)')]
    prio_list = [
     (
      '-1', _('Auto'))]
    ...
    advanced_lnb_ucsw_choices = [
     (
      '0', _('None'))] + [ (str(y), 'Input ' + str(y)) for y in range(1, 17) ]
    diseqc_mode_choices = [
     (
    ...
    positioner_mode_choices = [
     (
      'usals', _('USALS')), ('manual', _('manual'))]
    diseqc_satlist_choices = [
     (
      3600, _('automatic'), 1), (3601, _('nothing connected'), 1)] + nimmgr.satList
    ...
    def configLOFChanged(configElement):
        if configElement.value == 'unicable':
            ...
            def fillUnicableConf(sectionDict, unicableproducts, vco_null_check):
                for manufacturer in unicableproducts:
                    ...
                    for article in products:
                        ...
                        dictionlist = [
                         unicableproducts[manufacturer][article].get('diction')]
            ...
            for i in (1210, 1420, 1680, 2040, 984, 1020, 1056, 1092, 1128, 1164, 1256,
                      1292, 1328, 1364, 1458, 1494, 1530, 1566, 1602, 1638, 1716,
                      1752, 1788, 1824, 1860, 1896, 1932, 1968, 2004, 2076, 2112,
                      2148):
    ...
    def configDiSEqCModeChanged(configElement):
        ...
        if configElement.value == '1_2' and isinstance(section.longitude, ConfigNothing):
            ...
            section.powerThreshold = ConfigInteger(default=hw.get_device_name() == 'dm7025' and 50 or 15, limits=(0,
                                                                                                                  100))
    ...
    def createSatConfig(nim, x, empty_slots):
        try:
            ...
            nim.powerThreshold = ConfigInteger(default=hw.get_device_name() == 'dm8000' and 15 or 50, limits=(0,
                                                                                                              100))

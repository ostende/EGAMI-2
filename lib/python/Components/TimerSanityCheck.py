...
from time import localtimemktimegmtime
from ServiceReference import ServiceReference
from enigma import iServiceInformationeServiceCentereServiceReferencegetBestPlayableServiceReference
            ...
            if ci_timer and cihelper.ServiceIsAssigned(timer.service_ref.ref):
                ...
                if is_ci_timer_conflict == 1:
                    if ConflictTimer is None:
                        ConflictTimer = timer
                        ConflictTunerType = tunerType
            self.nrep_eventlist[idx] = (
             event[0], event[1], event[2], cnt, overlaplist[:])
            idx += 1

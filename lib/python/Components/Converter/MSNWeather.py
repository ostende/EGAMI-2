...
class MSNWeather(Converter, object):
    ...
    def getText(self):
        if self.mode == self.CITY:
            ...
            if self.mode == self.OBSERVATIONPOINT:
                return self.source.getObservationPoint()
            if self.mode == self.OBSERVATIONTIME:
                return self.source.getObservationTime()
            if self.mode == self.TEMPERATURE_CURRENT:
                return self.source.getTemperature_Current()
            if self.mode == self.FEELSLIKE:
                return self.source.getFeelslike()
            if self.mode == self.HUMIDITY:
                return self.source.getHumidity()
            if self.mode == self.WINDDISPLAY:
                return self.source.getWinddisplay()
            if self.mode == self.TEMPERATURE_HEIGH and self.index is not None:
                return self.source.getTemperature_Heigh(self.index)
            if self.mode == self.TEMPERATURE_LOW and self.index is not None:
                return self.source.getTemperature_Low(self.index)
            if self.mode == self.TEMPERATURE_HEIGH_LOW and self.index is not None:
                return self.source.getTemperature_Heigh_Low(self.index)
            if self.mode == self.TEMPERATURE_TEXT and self.index is not None:
                return self.source.getTemperature_Text(self.index)
            if self.mode == self.WEEKDAY and self.index is not None:
                return self.source.getWeekday(self.index, False)
            if self.mode == self.WEEKSHORTDAY and self.index is not None:
                return self.source.getWeekday(self.index, True)
            if self.mode == self.DATE and self.index is not None:

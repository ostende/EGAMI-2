...
from time import time as getTimelocaltimestrftime
                ...
                if config.usage.swap_time_display_on_osd.value == '1':
                    if remaining is not None:
                        if config.usage.swap_time_remaining_on_osd.value == '1':
                            ...
                            if config.usage.swap_time_remaining_on_osd.value == '2':
                                return sign_p + '%d  ' % (p / 60) + sign_r + ngettext(_('%d Min'), _('%d Mins'), r / 60) % (r / 60)
                            if config.usage.swap_time_remaining_on_osd.value == '3':
                ...
                if config.usage.swap_time_display_on_vfd.value == '1':
                    if remaining is not None:
                        if config.usage.swap_time_remaining_on_vfd.value == '1':
                            ...
                            if config.usage.swap_time_remaining_on_vfd.value == '2':
                                return sign_p + '%d  ' % (p / 60) + sign_r + ngettext(_('%d Min'), _('%d Mins'), r / 60) % (r / 60)
                            if config.usage.swap_time_remaining_on_vfd.value == '3':
                                return sign_r + '%d  ' % (r / 60) + sign_p + ngettext(_('%d Min'), _('%d Mins'), p / 60) % (p / 60)
                            return sign_r + ngettext(_('%d Min'), _('%d Mins'), r / 60) % (r / 60)

...
from enigma import iServiceInformationiPlayableService
...
class ServiceInfo(Converter, object):
    ...
    def __init__(self, type):
        ...
        self.type, self.interesting_events = {'HasTelext': (
           ...
           'HasHBBTV': (
                      self.HAS_HBBTV, (iPlayableService.evUpdatedInfo, iPlayableService.evHBBTVInfo)),
           ...
           'IsIPStream': (
                        self.IS_IPSTREAM, (iPlayableService.evUpdatedInfo,))
           }[type]
    ...
    def getText(self):
        ...
        if not info:
            ...
            if self.type == self.FRAMERATE:
                ...
                return (
                 video_rate, lambda x: '%d fps' % ((x + 500) / 1000))

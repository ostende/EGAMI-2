...
# [GCC 5.4.0 20160609]
# Embedded file name: /usr/lib/enigma2/python/Components/Converter/EGNetSpeedInfo.py
...
class EGNetSpeedInfo(Poll, Converter, object):
    RCL = 0
    TML = 1
    RCW = 2
    TMW = 3
    ...
    def updateNetSpeedInfoStatus(self):
        ...
        if flaglan == 1:
            ...
            if self.type == self.RC:
                return '%2.3f KB/s' % self.receive
            if self.type == self.TM:
                return '%2.3f KB/s' % self.transmit
            ...
            if self.type == self.TM_MB:
                return '%2.3f KB/s' % self.transmitmb

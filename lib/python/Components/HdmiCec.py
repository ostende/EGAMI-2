...
from enigma import eTimereHdmiCECeActionMap
from config import configConfigSelectionConfigYesNoConfigSubsectionConfigTextNoSaveConfigInteger
...
choicelist = []
for i in (10, 50, 100, 150, 250, 500, 750, 1000, 1500, 2000, 4000, 5000, 7000):
...
class HdmiCec():
    ...
    def wakeupMessages(self):
        if self.checkifPowerupWithoutWakingTv() == 'True':
            ...
            if os.path.exists('/usr/scripts/TvOn.sh'):
                Console().ePopen('/usr/scripts/TvOn.sh &')
    ...
    def standbyMessages(self):
        if config.hdmicec.control_tv_standby_skipnow.value:
            ...
            if os.path.exists('/usr/scripts/TvOff.sh'):
                Console().ePopen('/usr/scripts/TvOff.sh &')
    ...
    def standby(self):
        from Screens.Standby import StandbyinStandby
        if not inStandby:

...
from os import listdiropen as os_openclose as os_closewrite as os_writeO_RDWRO_NONBLOCK
from fcntl import ioctl
from boxbranding import getBoxTypegetBrandOEM
import struct
from config import configConfigSubsectionConfigIntegerConfigYesNoConfigTextConfigSlider
...
class InitInputDevices:
    ...
    def setupConfigEntries(self, device):
        ...
        if boxtype in ('maram9', 'classm', 'axodin', 'axodinc', 'starsatlx', 'genius',
            ...
            cmd = 'config.inputDevices.' + device + '.repeat = ConfigSlider(default=10, increment = 10, limits=(0, 500))'
        ...
        if boxtype in ('maram9', 'classm', 'axodin', 'axodinc', 'starsatlx', 'genius',
            ...
            cmd = 'config.inputDevices.' + device + '.delay = ConfigSlider(default=100, increment = 100, limits=(0, 5000))'
        elif boxtype == 'mbtwinplus':
...
class RcTypeControl:
    ...
    def __init__(self):
        if pathExists('/proc/stb/ir/rc/type') and pathExists('/proc/stb/info/boxtype') and getBrandOEM() not in ('gigablue',
                                                                                                                 'odin',
                                                                                                                 'ini',
                                                                                                                 'entwopia',
                                                                                                                 'tripledot'):

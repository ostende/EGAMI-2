...
from enigma import eDBoxLCDeTimereActionMap
from config import configConfigSubsectionConfigSelectionConfigSliderConfigYesNoConfigNothing
...
def InitLcd():
    if getBoxType() in ('evoslimse', 'evoslimt2c', 'valalinux', 'tmtwin4k', 'tmnanom3',
                        ...
                        'amiko8900', 'sognorevolution', 'arguspingulux', 'arguspinguluxmini',
                        'arguspinguluxplus', 'sparkreloaded', 'sabsolo', 'sparklx',
                        'gis8120', 'gb800se', 'gb800solo', 'gb800seplus', 'gbultrase',
                        'gbipbox', 'tmsingle', 'tmnano2super', 'iqonios300hd', 'iqonios300hdv2',
                        'optimussos1plus', 'optimussos1', 'vusolo', 'et4x00', 'et5x00',
                        'et6x00', 'et7000', 'et7100', 'mixosf7', 'mixoslumi', 'gbx1',
                        'gbx2', 'gbx3', 'gbx3h'):
        detected = False
    else:
    ...
    if detected:
        ...
        if not ilcd.isOled():
            ...
            if getBoxType() in 'dm900':
                ...
                standby_default = 1
        if getBoxType() in ('novatwin', 'novacombo', 'mixosf5', 'mixosf5mini', 'gi9196m',
                            'gi9196lite', 'zgemmas2s', 'zgemmash1', 'zgemmash2',
                            'zgemmass', 'zgemmahs', 'zgemmah2s', 'zgemmah2h', 'spycat'):
            config.lcd.standby = ConfigSlider(default=standby_default, limits=(0, 4))
            config.lcd.dimbright = ConfigSlider(default=standby_default, limits=(0,
                                                                                 4))
            ...
            config.lcd.dimbright = ConfigSlider(default=standby_default, limits=(0,
                                                                                 10))
            config.lcd.bright = ConfigSlider(default=SystemInfo['DefaultDisplayBrightness'], limits=(0,
                                                                                                     10))
        ...
        if SystemInfo['VFD_scroll_delay'] and getBoxType() not in ('ixussone', 'ixusszero'):
            ...
            config.usage.vfd_scroll_delay = ConfigSlider(default=150, increment=10, limits=(0,
                                                                                            500))
            config.usage.vfd_scroll_delay.addNotifier(scroll_delay, immediate_feedback=False)
            config.lcd.hdd = ConfigSelection([('0', _('No')), ('1', _('Yes'))], '1')
        else:
            config.lcd.hdd = ConfigNothing()
        if SystemInfo['VFD_initial_scroll_delay'] and getBoxType() not in ('ixussone',
                                                                           'ixusszero'):
        ...
        if SystemInfo['VFD_final_scroll_delay'] and getBoxType() not in ('ixussone',
                                                                         'ixusszero'):
        ...
        if getBoxType() == 'vuultimo':
            config.lcd.ledblinkingtime = ConfigSlider(default=5, increment=1, limits=(0,
                                                                                      15))
            config.lcd.ledblinkingtime.addNotifier(setLEDblinkingtime)
            config.lcd.ledbrightnessdeepstandby = ConfigSlider(default=1, increment=1, limits=(0,
                                                                                               15))
            ...
            config.lcd.ledbrightnessstandby = ConfigSlider(default=1, increment=1, limits=(0,
                                                                                           15))
            ...
            config.lcd.ledbrightness = ConfigSlider(default=3, increment=1, limits=(0,
                                                                                    15))

...
from time import timelocaltimemktime
from enigma import eTimereActionMap
import datetime
    ...
    def shouldSkip(self):
        ...
        if 'PowerTimerEntry' in `self`:
            if (self.timerType == 3 or self.timerType == 4) and self.autosleeprepeat != 'once':
                ...
                if self.begin >= time() and (self.timerType == 3 or self.timerType == 4) and self.autosleeprepeat == 'once':
                    return False
                if (self.timerType == 3 or self.timerType == 4) and self.autosleeprepeat == 'once' and self.state != TimerEntry.StatePrepared:
                    return True
